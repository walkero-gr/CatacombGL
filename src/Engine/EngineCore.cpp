...
constexpr uint8_t versionLevel = 7;
    ...
    m_savedGamesInDosFormat(m_game.GetSavedGameInDosFormatConfig())
{
    ...
    if (m_game.IsCatacomb3D())
    {
        ...
        const fs::path& gameDataPath = (game.GetId() == GameId::Catacomb3Dv100) ?
            configurationSettings.GetCVarString(CVarIdPathCatacomb3Dv100).Get() :
            configurationSettings.GetCVarString(CVarIdPathCatacomb3Dv122).Get();
...
void EngineCore::DrawScene(IRenderer& renderer)
{
    ...
    {
        if (!m_menu->IsActive() || m_game.IsCatacombAdventureSeries())
        {
    ...
    if (m_state == InGame)
    {
        ...
        else
        {
            ...
            {
                const uint16_t statusbarOffset = m_game.IsCatacomb3D() ? 144 : 120;
                ...
                if (m_game.IsCatacomb3D() && m_playerActions.GetShotPower() > 0 && m_gameTimer.GetTicksForPlayer() % 16 < 8)
                {
                    pictureIndex++;
                }
                const Picture* handPicture = m_game.GetEgaGraph()->GetMaskedPicture(pictureIndex);
                const int16_t offsetX = m_game.IsCatacomb3D() ? 92 : 120;
    ...
    if (((m_state == InGame || m_state == EnteringLevel || m_state == WarpCheatDialog || m_state == GodModeCheatDialog || m_state == FreeItemsCheatDialog || m_state == AutoMapDialog || m_state == VerifyGateExit || m_state == ExitGame) && (!m_menu->IsActive() || m_game.IsCatacombAdventureSeries())) || (m_state == Victory && m_game.IsCatacombAdventureSeries()))
    {
        ...
        if (m_state != Victory)
        {
            if (m_game.IsCatacombAdventureSeries())  // Status message and radar not in Catacomb 3-D
            {
    ...
    if (m_state == WarpCheatDialog)
    {
        ...
        const bool onScreenLevelIndexStartsAtZero = m_game.IsCatacombAdventureSeries();
    ...
    if (m_level != nullptr && m_level->GetPlayerActor()->IsDead() && m_playerInventory.GetPotions() > 0 && m_game.IsCatacombAdventureSeries())
    {
    ...
    if (m_menu->IsActive())
    {
        if (m_game.IsCatacombAdventureSeries())
        {
...
void EngineCore::EnterKeyReleased()
{
    if (!m_menu->IsActive())
    {
        ...
        else if (m_state == WarpCheatDialog)
        {
            ...
            // In Catacomb 3-D, the on-screen level indices start at 1 instead of 0.
            const int levelIndexInGameMaps = m_game.IsCatacomb3D() ? levelIndexOnScreen - 1 : levelIndexOnScreen;
...
bool EngineCore::Think()
{
    ...
    else if (m_state == InGame && m_game.IsCatacombAdventureSeries() && m_playerInput.IsKeyJustPressed(SDLK_F1))
    {
        m_state = Help;
    ...
    if (m_state == InGame)
    {
        ...
        if (!m_level->GetPlayerActor()->IsDead())
        {
            ...
            if (m_timeStampOfPlayerCurrentFrame > m_timeStampOfPlayerPreviousFrame)
            {
                ...
                {
                    const uint16_t projectileId = m_game.IsCatacomb3D() ? m_level->GetPlayerActor()->GetDecorateActor().projectileId + 1 : m_level->GetPlayerActor()->GetDecorateActor().projectileId;
                    const auto decorateProjectilePair = m_game.GetDecorateActors().find(projectileId);
                    if (decorateProjectilePair != m_game.GetDecorateActors().end())
                    {
    ...
    case ActionAttack:
    {
        if (actor->GetDecorateActor().damage > 0)
        {
            ...
            if (actor->WouldCollideWithActor(m_level->GetPlayerActor()->GetX(), m_level->GetPlayerActor()->GetY(), 1.1f))
            {
                if (!m_godModeIsOn)
                {
                    const uint8_t damage = (m_difficultyLevel == Easy && m_game.IsCatacombAdventureSeries()) ? actor->GetDecorateActor().damage / 2 : actor->GetDecorateActor().damage;
    ...
    case ActionForceField:
    {
        if (m_level->GetPlayerActor()->WouldCollideWithActor(actor->GetX(), actor->GetY(), actor->GetDecorateActor().size))
        {
            ...
            if (!m_godModeIsOn)
            {
                ...
                const uint8_t damage = (m_difficultyLevel == Easy && m_game.IsCatacombAdventureSeries()) ? baseDamage / 2 : baseDamage;
                m_level->GetPlayerActor()->Damage(damage);
                m_game.PlaySoundPlayerHurt(m_level->GetPlayerActor()->GetHealth());
                const uint8_t borderSignal = m_game.IsCatacombAdventureSeries() ? 12 : 5;
                const uint32_t borderFlashTime = (uint32_t)(16 * (1000.0 / 70.0));
        ...
        if (speed > 0.0f)
        {
                    ...
                    if (otherActor != nullptr)
                    {
                        if (action == ActionPlayerProjectile)
                        {
                                ...
                                (std::abs(basey - otherActor->GetY()) < size + otherActor->GetDecorateActor().size))
                            {
                                ...
                                {
                                    const bool isEasyModeFromAdventureSeries = (m_difficultyLevel == Easy && m_game.IsCatacombAdventureSeries());
            ...
            if (action == ActionMonsterProjectile)
            {
                if (m_level->GetPlayerActor()->WouldCollideWithActor(basex, basey, size))
                {
                    ...
                    {
                        const uint8_t baseDamage = (actor->GetTemp2() > 0) ? (uint8_t)actor->GetTemp2() : actor->GetDecorateActor().damage;
                        const uint8_t damage = (m_difficultyLevel == Easy && m_game.IsCatacombAdventureSeries() && baseDamage > 1) ? baseDamage / 2 : baseDamage;
                        m_level->GetPlayerActor()->Damage(damage);
                        m_game.PlaySoundPlayerHurt(m_level->GetPlayerActor()->GetHealth());
                        const uint8_t borderSignal = m_game.IsCatacombAdventureSeries() ? 12 : 5;
                        const uint32_t borderFlashTime = (uint32_t)(16 * (1000.0 / 70.0));
...
bool EngineCore::ClipWithTile(const uint16_t tileX, const uint16_t tileY, const float playerX, const float playerY)
{
    ...
    if (playerTouchesTile && m_level->IsSolidWall(tileX, tileY))
    {
        ...
        if (m_game.IsCatacomb3D() && m_state == InGame && !m_game.GetAudioPlayer()->IsPlaying())
        {
            ...
            if (m_level->IsBlockedDoor(tileX, tileY))
            {
                if (m_game.IsCatacombAdventureSeries())
                {
                    ...
                    m_game.PlaySoundHitGate();
                     ...
                     m_game.IsCatacomb3D()) // All doors in the Catacomb 3-D can be removed
            {
                // Open the door
...
void EngineCore::Thrust(const uint16_t angle, const float distance)
{
    ...
    if (!moveOk && m_game.IsCatacomb3D() && m_configurationSettings.GetCVarBool(CVarIdStickyWalls).IsEnabled())
    {
...
bool EngineCore::Chase(Actor* actor, const bool diagonal, const ChaseTarget target)
{
    ...
    while (move > 0.0f)
    {
        if (actor->GetState() == StateIdWalk || actor->GetState() == StateIdPain)
        {
            ...
            {
                // Melee attack
                const bool performRandomAttack = (m_game.IsCatacombAdventureSeries() && (rand() % (5000 / truncatedDeltaTimeInMs) == 0));
...
bool EngineCore::IsMusicSetToAdlibInConfiguration() const
{
    ...
    const uint8_t applicableMusicModeCVar = m_game.IsCatacomb3D() ? CVarIdMusicMode : CVarIdMusicModeAdventureTrilogy;
